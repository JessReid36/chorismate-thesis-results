...
def homo_lumo(p):
    L=open(p).readlines(); s=[i for i,l in enumerate(L) if 'ORBITAL ENERGIES' in l]
    ...
    return h,l
def loewdin(p,idx):
    L=open(p).readlines(); s=[i for i,l in enumerate(L) if 'LOEWDIN ATOMIC CHARGES' in l]
    ...
    return None
def rxyz(p):
    L=open(p).readlines(); n=int(L[0].split()[0]); return [(float(x.split()[1]),float(x.split()[2]),float(x.split()[3])) for x in L[2:2+n]]
def rtrj(p):
    L=open(p).readlines(); F=[]; i=0
    while i<len(L):
    ...
    return F
def rpc(p):
    L=open(p).readlines(); n=int(L[0].split()[0]); return [(float(x.split()[0]),float(x.split()[1]),float(x.split()[2]),float(x.split()[3])) for x in L[1:1+n]]
...
def mac(g,ch):
    ...
    for ai,a in enumerate(g):
        for qq,x,y,z in ch:
            v=dd(a,(x,y,z))
            if v<best[0]: best=(v,ai,qq)
    return best
def stat(p):
    t=open(p).read()
    if 'ORCA TERMINATED NORMALLY' not in t: return 'CRASH'
    if 'SCF NOT CONVERGED' in t: return 'SCF_FAIL'
    if 'HAS CONVERGED' in t or 'OPTIMIZATION RUN DONE' in t: return 'OPT_OK'
    return 'NOCONV'
def frmsd(a,b):
    return math.sqrt(sum(dd(a[i],b[i])**2 for i in range(len(a)))/len(a))
base=sys.argv[1] if len(sys.argv)>1 else os.getcwd()
print("%-12s %-8s %8s %7s %7s %8s %9s %6s %6s  %s"%("tag","status","HOMOeV","O3q","dO3q","minDist","trajMin","O3C4","C1C6","verdict"))
for tag in sorted(os.listdir(os.path.join(base,'runs'))):
    ...
    md,ai,q=mac(g,ch); tj=os.path.join(rd,'job_trj.xyz'); tmin=md
    ...
    print("%-12s %-8s %8s %7s %7s %8.3f %9.3f %6.3f %6.3f  %s"%(tag,st,
